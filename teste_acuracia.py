...
def acha_linha_4(pontos,linha_real):
	...
	if linha_real in ["422","298","864","326","778","908","455"]:
		f = open("C:/Users/IBM_ADMIN/Documents/tcc/testes/{linha}-{pontos}p-{dec}cd-{t}.txt".format(linha=linha_real,pontos=num_pontos,dec=n_dec,t=t_inicio),"a")
		if linha_real in dict_final:
			f.write(str(dict_final[linha_real])+"\n")
			if linha_real == linha_maior_pontuacao:
				f.write("s"+"\n")
			else:
				f.write("n"+"\n")
		else:
			f.write("0"+"\n"+"n"+"\n")
			f_error = open("C:/Users/IBM_ADMIN/Documents/tcc/testes/erro-{linha}-{pontos}p-{dec}cd-{t}.txt".format(linha=linha_real,pontos=num_pontos,dec=n_dec,t=t_inicio),"a")
			for p in pontos:
				lat = p[0]
				lng = p[1]
				f_error.write(str(lat)+";"+str(lng)+"\n")
			f_error.close()
		f.close()
...
def encontrar_linhas():
	with lock:
		for ordem in (ordem for ordem in dict_ordem if dict_ordem[ordem]["linha"]["confiavel"] == True):
				#se numero de pontos condiz com o necessario p/ analise
				if len(dict_ordem[ordem]["pontos"]) == num_pontos:
					linha = acha_linha_4(dict_ordem[ordem]["pontos"],dict_ordem[ordem]["linha"]["num"])
					if linha != "":
